            ...
            success: function(data)
            {
            	if(data)
            	{
            		...
            		if(json.res == 'full')
            		{
            			...
            			{
            				html+='<a href="#" onclick="info('+json.data[datos].CodiPers+',\''+json.data[datos].NombPers+'\')" class="list-group-item">';
            				html+='<h4 class="list-group-item-heading">ID:' + json.data[datos].CodiPers;
            				html+=' Nombre: ' + json.data[datos].NombPers+'</h4>';
            				html+='</a>';
            			}
            		}
            		else
...
function info(CodiPers,NombPers)
{
	alert("ID: " + CodiPers + " Nombre: " + NombPers);
}